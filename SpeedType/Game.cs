...
namespace SpeedType
{
    public class Game
    {
        ...
        public Game()
        {
            // Initialize the sentence provider and evaluator
            sentenceProvider = new SentenceProvider();
            evaluator = new Evaluator();

            // Initialize the game stats board with a size of 5
        ...
        private void ShowGameStats()
        {
            ...
            for (int i = 0; i < gameStats.Length; i++)
            {
                ...
                string wpm = gameStats[i].WPM.ToString("F2");
                string accuracy = gameStats[i].Accuracy.ToString() + "%";
                string timeTaken = gameStats[i].TimeTaken.ToString("F2");

                table.AddRow(rank, wpm, accuracy, timeTaken);
            }
